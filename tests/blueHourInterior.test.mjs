...
test('Blue Hour cast uses supported seats and clear gaming aisles',async()=>{
 const room=await load('interior-blue-hour');room.updateMatrixWorld(true);
 assert.ok(room.getObjectByName('interior-wall-left'));assert.ok(room.getObjectByName('interior-wall-back'));
 const people=[{id:'a-worker',role:'Cashier'},...Array.from({length:16},(_,i)=>({id:`visitor-${i}`}))];
 const places=placementsForInterior('casino',people);assert.equal(places.size,12);assert.equal(places.get('a-worker').id,'cashier');
 assert.deepEqual([...placementsForInterior('casino',[...people].reverse())],[...places]);
 const spots=[...places.values(),interiorPlayerSpot('casino')];
 for(const name of ['person','woman']){
  ...
  const actor=source.clone(true),arrival=new InteriorArrival(actor,'casino');
  for(let i=0;i<=40;i++){
