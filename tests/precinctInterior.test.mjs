...
import {precinctPlacements,poseInteriorOccupant,interiorPlayerSpot} from '../.runtime/frontend-test/interiorStaging.js';
...
test('Station seats support both rigs and standing visitors and entrance clear the furniture',async()=>{
 const room=await load('interior-precinct');room.updateMatrixWorld(true);
 assert.ok(room.getObjectByName('interior-wall-left'));assert.ok(room.getObjectByName('interior-wall-back'));
 const people=[{id:'a-worker',role:'Desk sergeant'},{id:'b-worker',role:'Detective'},...Array.from({length:16},(_,i)=>({id:`visitor-${i}`}))];
 const places=precinctPlacements(people);assert.equal(places.size,13);assert.equal(places.get('a-worker').id,'station-sergeant');
 assert.deepEqual([...precinctPlacements([...people].reverse())],[...places]);
 const spots=[...places.values(),interiorPlayerSpot('precinct')];
 for(const name of ['person','woman']){
  ...
  const actor=source.clone(true),arrival=new InteriorArrival(actor,'precinct');
  for(let i=0;i<=40;i++){
