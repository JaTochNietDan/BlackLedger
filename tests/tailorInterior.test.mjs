...
import {tailorPlacements,poseInteriorOccupant,interiorPlayerSpot} from '../.runtime/frontend-test/interiorStaging.js';
...
test('Tailor staff, customer chair and entrance fit both rigs without crossing displays or stairs',async()=>{
 const room=await load('interior-tailor');room.updateMatrixWorld(true);
 assert.ok(room.getObjectByName('interior-wall-left'));assert.ok(room.getObjectByName('interior-wall-back'));
 const people=[{id:'a-worker',role:'Cutter'},{id:'b-worker',role:'Finisher'},{id:'c-worker',role:'Fitting clerk'},...Array.from({length:16},(_,i)=>({id:`visitor-${i}`}))];
 const places=tailorPlacements(people);assert.equal(places.size,10);assert.equal(places.get('a-worker').id,'tailor-staff-0');
 assert.equal(places.get('b-worker').x,0);assert.equal(places.get('c-worker').x,1.5);
 assert.deepEqual([...tailorPlacements([...people].reverse())],[...places]);
 const spots=[...places.values(),interiorPlayerSpot('tailor')];
 for(const name of ['person','woman']){
  ...
  const actor=source.clone(true),arrival=new InteriorArrival(actor,'tailor');
  for(let i=0;i<=40;i++){
