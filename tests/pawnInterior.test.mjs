...
import {pawnPlacements,poseInteriorOccupant,interiorPlayerSpot} from '../.runtime/frontend-test/interiorStaging.js';
...
test('Pawnshop staff, customers and entrance clear the counter and stocked displays',async()=>{
 const room=await load('interior-pawn');room.updateMatrixWorld(true);
 assert.ok(room.getObjectByName('interior-wall-left'));assert.ok(room.getObjectByName('interior-wall-back'));
 const people=[{id:'a-worker',role:'Pawnbroker'},{id:'b-worker',role:'Counter clerk'},{id:'c-worker',role:'Valuer'},...Array.from({length:16},(_,i)=>({id:`visitor-${i}`}))];
 const places=pawnPlacements(people);assert.equal(places.size,9);assert.equal(places.get('a-worker').id,'pawn-staff-0');
 assert.equal(places.get('b-worker').x,2.2,'clerk belongs at register');assert.equal(places.get('c-worker').x,0,'valuer belongs at scales');
 assert.deepEqual([...pawnPlacements([...people].reverse())],[...places]);
 const spots=[...places.values(),interiorPlayerSpot('pawn')];
 for(const name of ['person','woman']){
  ...
  const actor=source.clone(true),arrival=new InteriorArrival(actor,'pawn');
  for(let i=0;i<=40;i++){
