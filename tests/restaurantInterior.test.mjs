...
import {restaurantPlacements,poseInteriorOccupant,interiorPlayerSpot} from '../.runtime/frontend-test/interiorStaging.js';
...
test('Restaurant seats support both rigs and standing visitors and entrance clear the furniture',async()=>{
 const room=await load('interior-restaurant');room.updateMatrixWorld(true);
 assert.ok(room.getObjectByName('interior-wall-left'));assert.ok(room.getObjectByName('interior-wall-back'));
 const people=[{id:'a-worker',role:'Waiter'},{id:'b-worker',role:'Waitress'},{id:'c-worker',role:'Cook'},{id:'d-worker',role:'Cellarman'},...Array.from({length:16},(_,i)=>({id:`visitor-${i}`}))];
 const places=restaurantPlacements(people);assert.equal(places.size,14);assert.equal(places.get('a-worker').id,'dining-service');
 assert.deepEqual([...restaurantPlacements([...people].reverse())],[...places]);
 const spots=[...places.values(),interiorPlayerSpot('restaurant')];
 for(const name of ['person','woman']){
  ...
  const actor=source.clone(true),arrival=new InteriorArrival(actor,'restaurant');
  for(let i=0;i<=40;i++){
