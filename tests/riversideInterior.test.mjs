...
import {placementsForInterior,poseInteriorOccupant,interiorPlayerSpot} from '../.runtime/frontend-test/interiorStaging.js';
...
test('Riverside seats support both rigs and standing visitors and entrance clear the furniture',async()=>{
 const room=await load('interior-riverside');room.updateMatrixWorld(true);
 ...
 const places=placementsForInterior('riverside',people);assert.equal(places.size,11);
 assert.deepEqual([...placementsForInterior('riverside',[...people].reverse())],[...places]);
 const spots=[...places.values(),interiorPlayerSpot('riverside')];
 for(const name of ['person','woman']){
  ...
  const actor=source.clone(true),arrival=new InteriorArrival(actor,'riverside');
  for(let i=0;i<=40;i++){
