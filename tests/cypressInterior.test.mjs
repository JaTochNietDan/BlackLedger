...
import {cypressPlacements,poseInteriorOccupant,interiorPlayerSpot} from '../.runtime/frontend-test/interiorStaging.js';
...
test('Cypress seats support both rigs and standing visitors and entrance clear the furniture',async()=>{
 const room=await load('interior-cypress');room.updateMatrixWorld(true);
 assert.ok(room.getObjectByName('interior-wall-left'));assert.ok(room.getObjectByName('interior-wall-back'));
 const people=Array.from({length:12},(_,i)=>({id:`visitor-${i}`}));
 const places=cypressPlacements(people);assert.equal(places.size,7);
 assert.deepEqual([...cypressPlacements([...people].reverse())],[...places]);
 const spots=[...places.values(),interiorPlayerSpot('estate')];
 for(const name of ['person','woman']){
  ...
  const actor=source.clone(true),arrival=new InteriorArrival(actor,'estate');
  for(let i=0;i<=40;i++){
