...
import {docksPlacements,poseInteriorOccupant,interiorPlayerSpot} from '../.runtime/frontend-test/interiorStaging.js';
...
test('Pier dispatch, visitors and entry path clear stacked freight and equipment',async()=>{
 const room=await load('interior-docks');room.updateMatrixWorld(true);
 assert.ok(room.getObjectByName('interior-wall-left'));assert.ok(room.getObjectByName('interior-wall-back'));
 const people=[{id:'a-worker',role:"Ship's clerk"},...Array.from({length:16},(_,i)=>({id:`visitor-${i}`}))];
 const places=docksPlacements(people);assert.equal(places.size,9);assert.equal(places.get('a-worker').id,'dock-dispatch');
 assert.deepEqual([...docksPlacements([...people].reverse())],[...places]);
 const spots=[...places.values(),interiorPlayerSpot('docks')];
 for(const name of ['person','woman']){
  ...
  const actor=source.clone(true),arrival=new InteriorArrival(actor,'docks');
  for(let i=0;i<=40;i++){
