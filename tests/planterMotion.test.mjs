...
test('planter exits the authored vestibule without crossing the door or masonry',async()=>{
 for(const name of ['monarch','tavern','tenement','mercer-court','casino','civic','shop','warehouse','bluehour','goldenlily','papermoon','mariner','undertaker','filling'])for(const person of ['person','woman']){
  const building=await model(name),actor=await model(person),cast=new CityPlanter(actor);
  if(name==='filling')building.rotation.y=Math.PI;
