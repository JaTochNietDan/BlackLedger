...
import {cabstandPlacements,poseInteriorOccupant,interiorPlayerSpot} from '../.runtime/frontend-test/interiorStaging.js';
...
test('Cab dispatch, drivers and entry path clear office furniture',async()=>{
 const room=await load('interior-cabstand');room.updateMatrixWorld(true);
 assert.ok(room.getObjectByName('interior-wall-left'));assert.ok(room.getObjectByName('interior-wall-back'));
 const people=[{id:'a-worker',role:"Dispatcher"},...Array.from({length:16},(_,i)=>({id:`visitor-${i}`}))];
 const places=cabstandPlacements(people);assert.equal(places.size,9);assert.equal(places.get('a-worker').id,'cab-dispatch');
 assert.deepEqual([...cabstandPlacements([...people].reverse())],[...places]);
 const spots=[...places.values(),interiorPlayerSpot('cabstand')];
 for(const name of ['person','woman']){
  ...
  const actor=source.clone(true),arrival=new InteriorArrival(actor,'cabstand');
  for(let i=0;i<=40;i++){
