...
import {chapelPlacements,poseInteriorOccupant,interiorPlayerSpot} from '../.runtime/frontend-test/interiorStaging.js';
...
test('Chapel visitors have supported seats and a clear reception and entrance',async()=>{
 const room=await load('interior-chapel');room.updateMatrixWorld(true);
 assert.ok(room.getObjectByName('interior-wall-left'));assert.ok(room.getObjectByName('interior-wall-back'));
 const people=[{id:'a-worker',role:'Undertaker'},...Array.from({length:16},(_,i)=>({id:`visitor-${i}`}))];
 const places=chapelPlacements(people);assert.equal(places.size,9);assert.equal(places.get('a-worker').id,'chapel-reception');
 assert.deepEqual([...chapelPlacements([...people].reverse())],[...places]);
 const spots=[...places.values(),interiorPlayerSpot('chapel')];
 for(const name of ['person','woman']){
  ...
  const actor=source.clone(true),arrival=new InteriorArrival(actor,'chapel');
  for(let i=0;i<=40;i++){
