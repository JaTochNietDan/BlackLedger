...
for(const name of ['tavern','tenement','casino','warehouse','civic','shop','monarch','bluehour','goldenlily','papermoon'])test(`${name} door opens onto a supported person-sized vestibule`,async()=>{
 ...
 const entry=model.getObjectByName('entrance-threshold');assert.ok(entry);model.updateMatrixWorld(true);
 const front=entry.getWorldPosition(new THREE.Vector3()).z+.08;
 const ray=new THREE.Raycaster();
 const blocked=(x,y)=>{ray.set(new THREE.Vector3(x,y,front-1),new THREE.Vector3(0,0,1));ray.far=3.2;return ray.intersectObject(model,true).length>0;};
 ...
 for(const z of [front,front+.5,front+1,front+1.5]){
