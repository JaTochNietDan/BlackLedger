...
import {CitySuppression,waterArc} from '../.runtime/frontend-test/city3dSuppression.js';
...
test('suppression requires an attending crew, freezes motion and releases hose geometry at extinguishing',()=>{
 ...
 const water=effect.root.children[0].children.find(o=>o instanceof THREE.Points),before=Array.from(water.geometry.attributes.position.array);
 update(500,false);assert.deepEqual(Array.from(water.geometry.attributes.position.array),before);
 let disposed=0;effect.root.traverse(o=>{if(o.geometry)o.geometry.addEventListener('dispose',()=>disposed++);});
 update(660);assert.equal(effect.inspect().length,0);assert.equal(disposed,4);assert.equal(a.getObjectByName('arm1').rotation.x,0);
 effect.dispose();
});
