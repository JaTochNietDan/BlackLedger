...
func TestTheDrumsTravelToWhereTheyStop(t *testing.T) {
	...
	for _, required := range []string{"drumFaces(p.strip,p.line,p.pulled)", "drumRun(p.strip,windows[i],i,14)", "offset=14*Math.pow(1-progress,3)", "cell=surface.height/3", "(at-offset)*cell", "texture.needsUpdate=true"} {
		if !holds(cabinet, required) {
			t.Errorf("3D reel travel lost %q", required)
		}
	}
	if !holds(source(t, "src/cards.ts"), "const out = [...window];") {
		t.Fatal("run no longer lands on committed faces")
	}
	if !holds(source(t, "src/Tables.tsx"), "const animate=motion&&!reduced") {
		t.Fatal("machine ignores motion preference")
	}
