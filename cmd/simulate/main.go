...
func main() {
	...
	profiles := flag.String("strategies", "worker,investor,defiant,reckless,thief,smuggler,racketeer,publican,magpie,distiller", "comma-separated player policies")
	...
	for _, p := range strategies {
		if p != "worker" && p != "investor" && p != "reckless" && p != "defiant" && p != "diplomat" && p != "thief" && p != "smuggler" && p != "racketeer" && p != "publican" && p != "magpie" && p != "distiller" {
