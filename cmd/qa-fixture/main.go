...
func main() {
	...
	err = s.Change(func(w *core.World) error {
		if scenario == "headquarters" || scenario == "crew-orders" {
			w.Player.Location, w.Player.Respect, w.Player.Cash = "laundry", 45, 6000
			...
			if scenario == "crew-orders" {
				...
				for _, id := range []string{"leo", "mara"} {
					n := w.NPC(id)
					n.Location = "laundry"
					n.Heading = ""
					n.Arrives = 0
		...
		if scenario == "household-work" || scenario == "household-work-owner" {
			candidate := core.New(81)
			candidate.Player.Location = "room"
			candidate.Event, candidate.Plots, candidate.Tasks = nil, nil, nil
			n := candidate.NPC("mara")
			n.Home, n.Purse = "room", 500
			n.Location, n.Heading, n.Arrives, n.Sets = "room", "", 0, 0
			if scenario == "household-work-owner" {
				candidate.Properties["room"].Owner = "player:1"
			}
			candidate.SettleHousing()
			candidate.SettleApartments()
			*w = *candidate
			return nil
		}
		...
		if scenario == "burglary" || scenario == "burglary-search" || scenario == "estate-search" {
			address := "mercercourt"
			if scenario == "estate-search" {
				address = "estate"
			}
			...
			w.Event, w.Plots, w.Tasks = nil, nil, nil
			n := w.NPC("mara")
			n.Home, n.Location, n.Heading = address, "bar", ""
			n.Purse = 90
			n.Faction = ""
			w.MeetPerson(n.ID)
			...
			w.SettleApartments()
			w.RNG = 1
			if scenario == "burglary-search" || scenario == "estate-search" {
				next, err := core.Execute(w, core.Command{RequestID: core.ID(), Revision: w.Revision, Kind: "burgle:mara", Target: address})
				if err != nil {
					return err
				}
				*w = *next
		...
		if scenario == "casino-interior" || scenario == "cemetery-interior" || scenario == "crematorium-interior" || scenario == "mortuary-interior" || scenario == "cabstand-interior" || scenario == "docks-interior" || scenario == "chapel-interior" || scenario == "herald-interior" || scenario == "poolhall-interior" || scenario == "tailor-interior" || scenario == "pawn-interior" || scenario == "riverside-interior" || scenario == "butcher" || scenario == "garage-interior" || scenario == "restaurant-interior" || scenario == "exchange-interior" || scenario == "precinct-interior" {
			address, role := "butcher", "Butcher"
			if scenario == "casino-interior" {
				address, role = "casino", "Cashier"
			}
			if scenario == "cemetery-interior" {
				address, role = "cemetery", "Cemetery clerk"
			}
			if scenario == "crematorium-interior" {
				address, role = "crematorium", "Memorial clerk"
			}
			if scenario == "mortuary-interior" {
				address, role = "mortuary", "Receiving attendant"
			}
			if scenario == "cabstand-interior" {
				address, role = "cabstand", "Dispatcher"
			}
			if scenario == "docks-interior" {
				address, role = "docks", "Ship's clerk"
			}
			if scenario == "chapel-interior" {
				address, role = "chapel", "Undertaker"
			}
			if scenario == "herald-interior" {
				address, role = "herald", "Editor"
			}
			if scenario == "poolhall-interior" {
				address, role = "poolhall", "Marker"
			}
			if scenario == "tailor-interior" {
				address, role = "tailor", "Cutter"
			}
			if scenario == "pawn-interior" {
				address, role = "pawn", "Pawnbroker"
			}
			if scenario == "riverside-interior" {
				address, role = "riverside", "Resident"
			...
			if scenario == "precinct-interior" {
				address, role = "precinct", "Desk sergeant"
			}
			if scenario == "exchange-interior" {
				address, role = "market", "Clerk"
			}
			if scenario == "restaurant-interior" {
				address, role = "restaurant", "Waiter"
			}
			if scenario == "garage-interior" {
				address, role = "garage", "Mechanic"
			}
			w.Player.Location, w.Player.Cash = address, 6000
			w.District, w.Minute = 9, 600
			w.Event, w.Plots, w.Tasks = nil, nil, nil
			for i := 0; i < 6 && i < len(w.NPCs); i++ {
				n := &w.NPCs[i]
				n.Location, n.Heading, n.Arrives, n.Sets = address, "", 0, 0
				if i == 0 {
					n.Role = role
				}
				w.MeetPerson(n.ID)
		...
		if scenario == "apartment-portfolio" || scenario == "apartments" || scenario == "ashbury" {
			...
			w.SettleApartments()
			if scenario == "apartment-portfolio" {
				counts := map[string]int{}
				for i := range w.Apartments {
					u := &w.Apartments[i]
					if u.Building != "mercercourt" && u.Building != "apartment" {
						continue
					}
					key := u.Building + ":occupied"
					limit := 2
					if u.Resident == "" {
						key, limit = u.Building+":vacant", 1
					}
					if counts[key] >= limit {
						continue
					}
					counts[key]++
					u.Owner = fmt.Sprintf("player:%d", w.Life)
					if n := w.NPC(u.Resident); n != nil {
						p := w.Properties[u.Building]
						if p.Rents == nil {
		...
		if scenario == "home-strike" || scenario == "estate-strike" || scenario == "lodging-strike" {
			address := "mercercourt"
			if scenario == "lodging-strike" {
				address = "room"
			}
			if scenario == "estate-strike" {
				address = "estate"
			}
			w.Player.Location, w.District = address, 9
			w.Player.Cash, w.Player.Respect, w.Player.Weapon = 6000, 60, 1
			w.Event, w.Plots, w.Tasks = nil, nil, nil
			victim := w.NPC("mara")
			victim.Home, victim.Location, victim.Heading = address, address, ""
			victim.Arrives, victim.Sets, victim.Weapon, victim.Sore = 0, 0, 0, 0
			w.RNG, w.WorldRNG = 1, 1
			next, err := core.Execute(w, core.Command{RequestID: core.ID(), Revision: w.Revision, Kind: "home_strike:mara", Target: address})
			if err != nil {
				return err
			}
			*w = *next
			if !w.NPC("mara").Dead {
				return fmt.Errorf("home strike fixture did not succeed")
			}
			return nil
		}
		if weaponFixture {
