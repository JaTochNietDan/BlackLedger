...
export class CityStairDescent {
 ...
 update(seconds:number){
  const beat=THREE.MathUtils.clamp(seconds/.65,0,6),index=Math.min(5,Math.floor(beat)),u=beat-index;
  ...
  this.actor.position.set(0,(feet[0].y+feet[1].y-lift)/2-.25,(feet[0].z+feet[1].z)/2);
