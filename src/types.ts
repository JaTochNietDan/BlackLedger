...
export interface VisualCue {
  ...
  strike?: {setting?: 'home' | 'interior'; variant: 'back-of-head' | 'close-shot' | 'burst' | 'close-quarters'; victim: {id: string; name: string}};
...
export interface Snapshot {
  ...
  pool_opponents?:PoolOpponent[];
	apartment_market?: {id:string;building:string;number:number;address:string;owned:boolean;home:boolean;available:boolean;locked?:boolean;owner:string;resident:string;asking:number;offer:number;daily_rent:number;rent_paid_today?:number|null;rent_arrears?:number;vacant?:boolean;neighborhood_index?:number}[];
