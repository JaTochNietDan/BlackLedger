...
export type CardMove={card?:Card;row:number;index:number;from:number[];to:number[];delay:number;duration:number};
...
export function planCards(before:PublicHand,after:PublicHand,animate:boolean){
 ...
 for(let row=0;row<2;row++)for(let i=0;i<next[row].length;i++){
  ...
  cards.push({card,row,index:i,to,from:changed?[.94,1.12,-.30]:cardPosition(i,old[row].length,row),delay:animate&&changed?(fresh?(i*2+(row===1?0:1)):dealt++)*380:0,duration:animate?(changed?480:240):0});
 }
...
export function cardPose(move:CardMove,elapsed:number){
 ...
 return {visible:elapsed>=move.delay,position:t===1?[...move.to]:move.to.map((v,i)=>move.from[i]+(v-move.from[i])*ease+(i===1?.12*Math.sin(Math.PI*t):0))};
}
