...
const paths: Record<string, string> = {
  ...
  held: 'M4 3v18M9 3v18M15 3v18M20 3v18M2 8h20M2 16h20',
  market: 'M4 9h16l-1 11H5zM4 9 6 4h12l2 5M9 13v4m6-4v4',
};
...
export function portrait(id: string, size = '') {
  ...
  const cx = 32;
  return (
    `<svg class="portrait ${size}" viewBox="0 0 64 78" aria-hidden="true">` +
    `<defs><radialGradient id="g-${id.replace(/[^a-z0-9]/gi, '')}" cx="${light > 0 ? 0.32 : 0.68}" cy=".3" r=".9">` +
    ...
    // hair
    (style === 0
      ? `<path d="M${cx - wide / 2 - 1} 26 q1-19 ${wide / 2 + 1} -19 t${wide / 2 + 1} 19 q-3-10 -${wide / 2 + 1} -10 q-${wide / 2 + 1} 0 -${wide / 2 + 1} 10z" fill="${hair}"/>`
      : style === 1
        ? `<path d="M${cx - wide / 2 - 1} 24 q2-18 ${wide / 2 + 1} -18 q6 0 8 6 q-9-1 -14 6 q-3 2 -3 6z" fill="${hair}"/>`
        : style === 2
          ? `<path d="M${cx - wide / 2 - 1} 27 q0-20 ${wide / 2 + 1} -20 t${wide / 2 + 1} 20 q-2-13 -${wide / 2 + 1} -13 t-${wide / 2 + 1} 13z" fill="${hair}"/><path d="M${cx - wide / 2 - 2} 26 q-2 10 1 16 q-4-6 -3-16z" fill="${hair}"/><path d="M${cx + wide / 2 + 1} 26 q2 10 -1 16 q4-6 3-16z" fill="${hair}"/>`
          : style === 3
            ? `<path d="M${cx - wide / 2} 22 q4-15 ${wide / 2} -15 t${wide / 2} 15 q-4-8 -${wide / 2} -8 t-${wide / 2} 8z" fill="${hair}"/>`
            : `<path d="M${cx - wide / 2 - 1} 25 q1-18 ${wide / 2 + 1} -18 t${wide / 2 + 1} 18 q-1-8 -6-11 q-8 5 -16 3 q-3 3 -3 8z" fill="${hair}"/>`) +
    // brow, eyes, mouth
    (brow === 0
      ? ''
      : `<path d="M${cx - 9} 27h6m6 0h6" stroke="#2a231c" stroke-width="${1 + brow * 0.5}" opacity=".8"/>`) +
    ...
    (hat
      ? `<path d="M${cx - wide / 2 - 5} 20 h${wide + 10} l-3-4 h-${wide + 4}z" fill="#1e1c19"/>` +
...
export function building(p: Place, mini = false) {
  let x = mini ? 100 : p.x,
    y = mini ? 75 : p.y,
...
export function citySVG(state: Snapshot, selected: string) {
  ...
  for (let i = 0; i < 55; i++) {
    let x = 75 + ((i * 137) % 920),
      y = 155 + ((i * 83) % 470);
    if (
      state.locations.some(p => Math.abs(p.x - x) < 80 && Math.abs(p.y - y) < 65) ||
...
function pressHash(seed: string) {
  let h = 2166136261;
  for (const c of seed) {
    h ^= c.charCodeAt(0);
    h = Math.imul(h, 16777619);
...
function halftone(seed: number, density: number) {
  ...
  for (let y = 0; y < 11; y++)
    for (let x = 0; x < 16; x++) {
      ...
      const r = 0.7 + ((seed >> ((x * 2 + y) % 17)) & 3) * 0.35;
...
function pressShape(kind: string, subject: PressSubject, seed: number) {
  ...
  switch (subject.kind) {
    case 'person':
      return (
        `<g fill="#14100c"><path d="M48 74q0-14 16-18l-3-6q-6-4-6-13 0-13 13-13t13 13q0 9-6 13l-3 6q16 4 16 18z"/></g>` +
        (kind === 'killing'
          ? `<g stroke="#14100c" stroke-width="3" fill="none"><path d="M22 70l24-16m0 16L22 54"/></g>`
          : '')
      );
    case 'place':
      return (
        `<g fill="#14100c"><path d="M20 74V34l25-14 25 14v40z"/><path d="M78 74V46l14-8 14 8v28z"/></g>` +
        ...
        (kind === 'attack'
          ? `<g stroke="#14100c" stroke-width="3" fill="none"><path d="M45 20l-9 16 12 4-10 14"/></g>`
          : '') +
        (kind === 'police'
          ? `<g fill="#14100c"><rect x="8" y="62" width="30" height="12" rx="3"/><circle cx="16" cy="76" r="4"/><circle cx="31" cy="76" r="4"/></g>`
          : '')
      );
    default:
      return (
        `<g fill="#14100c"><path d="M6 74V44l12-7v37zM24 74V34l16-9v49zM46 74V22l18-10v62zM70 74V38l14-8v44zM90 74V48l14-7v33z"/></g>` +
...
export function pressPlate(kind: string, subject: PressSubject, headline: string) {
  ...
  const sky = (seed % 3) * 0.06;
  return (
    `<svg class="press-plate" viewBox="0 0 112 80" role="img" aria-label="${esc(subject.name)}">` +
    `<rect width="112" height="80" fill="#efe7d8"/>` +
    halftone(seed, 0.34 + sky) +
