...
function lit(kind: string, t: number) {
  if (kind === 'gunfight' || kind === 'killing') {
    const firing = t < 0.45 && Math.floor(t * 14) % 2 === 0;
    return {flash: firing ? 0.85 : 0, glow: '#ffe6a8', shake: 0};
  }
  if (kind === 'explosion') {
    return {flash: Math.max(0, 0.95 - t * 1.4), glow: '#ffcf7a', shake: Math.max(0, 1 - t * 3) * 7};
  }
  if (kind === 'raid' || kind === 'arrest') {
    return {flash: 0.18 + Math.abs(Math.sin(t * 22)) * 0.22, glow: '#e0705c', shake: 0};
  }
  return {flash: 0.1, glow: '#d6b77c', shake: 0};
}
...
export function CityStreet({
  ...
  const [figures, setFigures] = useState<
    {id: string; name: string; left: number; top: number; yours?: boolean}[]
  >([]);
  const place = () => {
  ...
  useEffect(() => {
    if (!spotlight) return;
    document
      .querySelector(`[data-place="${spotlight.id}"]`)
  ...
  return (
    <div
      ref={stage}
      className={'city-street' + (spotlight ? ' watching' : '')}
    ...
    >
      ...
      {figures.map(f => (
        <div
          key={f.id}
          className={'walker-figure' + (f.yours ? ' yours' : '')}
      ...
      {!!street.length && (
        <section className="street-out" aria-label="People on the street">
          <h3>OUT ON THE STREET</h3>
          <div className="street-out-run">
            {street.map(j => (
              <div className={'walker' + (j.yours ? ' yours' : '')} key={j.id}>
                ...
                <div>
                  ...
                  <small className="walker-why">
                    {j.because} · {j.minutes} min out
                  </small>
                  <span className="walker-track">
                    <i style={{left: (j.progress * 100).toFixed(0) + '%'}} />
                  </span>
                </div>
              </div>
            ))}
      ...
      {districts.map((name, d) => {
        ...
        return (
          <section className={'street-block' + (shut ? ' locked' : '')} key={name}>
            <h3>
              {name}
              {shut && <span>Not open to you yet</span>}
            </h3>
            <div className="street-run">
              {places.map(p => {
                ...
                const shot =
                  spotlight && spotlight.id === p.id ? lit(spotlight.kind, spotlight.t) : null;
                return (
                  <button
                    key={p.id}
                    data-place={p.id}
                  ...
                  >
                    <span
                      ...
                      style={
                        shot && shot.shake
                          ? {
                              transform: `translate(${(Math.random() - 0.5) * shot.shake}px,${(Math.random() - 0.5) * shot.shake}px)`,
                            }
                          : undefined
                      }
                    >
                    ...
                    <span className="front-plate">
                      ...
                      {p.id !== here && !shut && (
                        <small className="front-travel">{p.travel_note}</small>
                      )}
                      {p.note && (
                        <small className={'front-note' + (p.note_warn ? ' warning' : '')}>
                          {p.note}
                        </small>
                      )}
                      {!!people.length && (
                        <span className="front-who">
                          {people.slice(0, FACES).map(w => (
                            <Portrait key={w.id} id={w.id} size="tiny" />
                          ))}
                          <i>{people.length}</i>
                        </span>
                      )}
      ...
      <p className="street-note">
        Pick an address to inspect it, and see what the journey costs before you commit to it. You
        can only step inside the one you are standing in.{' '}
        <a href="/street-study.html" target="_blank" rel="noreferrer">
          See the Old Harbor block painted →
        </a>
      </p>
    </div>
  );
