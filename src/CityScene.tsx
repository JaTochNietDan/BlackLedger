...
export function CityScene({state,selected,onSelect}:{state:Snapshot;selected:string;onSelect:(id:string)=>void}){
 ...
 async function update(){const app=appRef.current;if(!app){timer=window.setTimeout(update,30);return}try{let svg=citySVG(state,selected).replace('<svg class="map"','<svg xmlns="http://www.w3.org/2000/svg" width="1620" height="1110" class="map"');svg=svg.replace('<defs>','<defs><style>.property-label{font-family:Arial;font-size:13px;fill:#e1ddc9;paint-order:stroke;stroke:#192622;stroke-width:4px}.property-sub{font-family:Arial;font-size:9px;fill:#beaa7c}.district{font-family:Arial;font-size:14px;letter-spacing:4px;fill:#95a18d;opacity:.6}.locked{opacity:.48}.selected .footprint{stroke:#d6b77c;stroke-width:2}.waterline{stroke:#5b7777;stroke-width:1;opacity:.23}</style>');
 ...
 let cleanup=()=>{};update();return()=>{disposed=true;clearTimeout(timer);cleanup()}},[state,selected]);
