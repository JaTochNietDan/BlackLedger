...
export function interiorSVG(place: Place, painted = false) {
  ...
  if (painted) return '';
  return (
    `<svg class="interior" viewBox="0 0 520 360" role="presentation">` +
...
export function roomLight(minute: number): RoomLight {
  const dark = nightness(minute),
    gold = goldenness(minute);
