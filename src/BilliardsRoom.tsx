...
import {poolPockets} from './billiards';
...
export function BilliardsRoom({pool,busy,motion,act}:{pool:PoolState;busy:boolean;motion:boolean;act:(c:Command)=>void}){
 ...
 return <section className="billiards-room" role="dialog" aria-modal="true" aria-label="Green Baize billiards">
  ...
  <BilliardsTable3D pool={pool} angle={angle*Math.PI/180} motion={motion} onPlaying={setPlaying}/>
  <div className="pool-controls">
   <p aria-live="polite">{playing?'Balls in motion…':pool.unavailable||pool.outcome||(pool.ball_in_hand?'Place the cue ball to break.':'Choose your line and play.')} </p>
   {!pool.settled&&<fieldset disabled={!canPlay}>
    {!playerTurn?<button onClick={()=>act({kind:'pool_opponent'})}>Watch {pool.opponent_name} play</button>:pool.break_choices.length?<div>{pool.break_choices.map(c=><button key={c.id} onClick={()=>act({kind:'pool_decide',choice:c.id})}>{c.label}</button>)}</div>:pool.ball_in_hand?<div className="pool-inputs">
     ...
     <label>From near end (m)<input type="number" min={pool.radius} max={pool.behind_head_string?pool.length/4:pool.length-pool.radius} step={.01} value={y} onChange={e=>setY(Number(e.target.value))}/></label>
     <button onClick={()=>act({kind:'pool_place',pool:{x,y}})}>Place cue ball{pool.behind_head_string?' behind head string':''}</button>
    </div>:<>
