...
import {HOME_STRIKE_ORIGIN} from './homeStrike';
...
export function HomeStrikeScene({cue,world,motion,overlay,onDone}:{cue:VisualCue;world:Snapshot;motion:boolean;overlay:ReactNode;onDone:(id:string)=>void}){
 ...
 const [status,setStatus]=useState('Opening the apartment…');
 useEffect(()=>{
  ...
  el.append(renderer.domElement);renderer.domElement.setAttribute('aria-label','Recorded attack inside the resident’s apartment');
  const camera=new THREE.PerspectiveCamera(43,1,.1,60);camera.position.set(5.7,6.4,10.8);camera.lookAt(0,.7,2);
  ...
  Promise.all([loader.loadAsync('/art/models/interior-flat.glb'),actor(cue.attacker!.id),actor(cue.strike!.victim.id),weaponName?loader.loadAsync(`/art/models/${weaponName}.glb`):Promise.resolve(undefined)]).then(([room,attacker,victim,weapon])=>{
   ...
   cast.root.position.set(HOME_STRIKE_ORIGIN.x,0,HOME_STRIKE_ORIGIN.z);scene.add(cast.root);
   scene.traverse(o=>{if(o instanceof THREE.Mesh){o.castShadow=true;o.receiveShadow=true;}});
   start=performance.now();setStatus('');
  }).catch(()=>{if(!dead){setStatus('The apartment scene could not load. The recorded result is available below.');finished=true;latest.current.onDone(cue.id);}});
  ...
  const tick=(now:number)=>{
   ...
   for(let i=0;i<20;i++){
    ...
    temp.position.set(HOME_STRIKE_ORIGIN.x+drop.x,drop.y,HOME_STRIKE_ORIGIN.z+drop.z);temp.scale.setScalar(weaponName?drop.size:0);temp.updateMatrix();droplets.setMatrixAt(i,temp.matrix);
   }
