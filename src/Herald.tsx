...
export function PressCut({
  ...
  return (
    <figure className={'cut' + (picture ? ' photographed' : '')}>
      ...
      {picture ? (
        ...
        <span
          className="plate"
          dangerouslySetInnerHTML={{__html: pressPlate(kind, subject, headline)}}
        />
      )}
  ...
  return (
    <>
      <div className="issue-bar">
        <button className="plain" disabled={!newer} onClick={() => setAt(a => a - 1)}>
          ← Later
        </button>
        <div className="issue-which">
          <b>{issue.current ? 'Today’s edition' : `Day ${issue.day}`}</b>
          <small>
            {issue.dateline} · {issue.count} {issue.count === 1 ? 'story' : 'stories'}
            {issue.mine ? '' : ` · life ${issue.life}`}
          </small>
        </div>
        <button className="plain" disabled={!older} onClick={() => setAt(a => a + 1)}>
          Earlier →
        </button>
      ...
      {showAll && (
        <ul className="issue-index">
          {issues.map((i, n) => (
            <li key={i.life + '-' + i.day}>
              <button
                className={'plain' + (n === at ? ' picked' : '') + (i.mine ? '' : ' past-life')}
                onClick={() => {
                  setAt(n);
                  setShowAll(false);
                }}
              >
                ...
                <small>
                  {i.count} {i.count === 1 ? 'story' : 'stories'}
                  {i.mine ? '' : ` · life ${i.life}`}
                </small>
              </button>
            </li>
      ...
      {lives.length > 1 && !issue.mine && (
        <p className="past-life-note">
      ...
      <div className="paper-sheet">
        <div className="paper">
          ...
          <div className="columns">
            {issue.stories
              .filter(s => s.kind !== 'civic' && s.kind !== 'obituary')
              .map((s, i) => (
                <article className={i === 0 ? 'lead' : ''} key={s.id}>
                  {s.subject && (
                    <PressCut kind={s.kind} subject={s.subject} headline={s.headline} />
                  )}
                  <h2>{s.headline}</h2>
          ...
          {issue.stories.some(s => s.kind === 'obituary') && (
            <div className="obituaries">
              <h3>Obituaries</h3>
              {issue.stories
                .filter(s => s.kind === 'obituary')
                .map(s => (
                  <article key={s.id}>
                    <h4>{s.headline}</h4>
                    <p>{s.body}</p>
          ...
          {issue.stories.some(s => s.kind === 'civic') && (
            <div className="city-page">
              <h3>The city in brief</h3>
              <div className="briefs">
                {issue.stories
                  .filter(s => s.kind === 'civic')
                  .map(s => (
                    <article key={s.id}>
                      <h4>{s.headline}</h4>
                      <p>{s.body}</p>
