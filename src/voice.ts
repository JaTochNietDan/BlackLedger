 ...
 */
export function speaking(status:string){return status===PREPARING||status===SPEAKING}

export const PREPARING='Preparing voice…',SPEAKING='Speaking…',IDLE='Read aloud',REPLAY='Replay voice',RETRY='Retry voice';
...
export class VoicePlayer {
 ...
 stop(){this.epoch++;this.request?.abort();this.request=null;if(this.playing){this.playing.pause();this.playing.dispose();this.playing=null}this.deps.status(IDLE)}
 async speak(event:string){this.stop();const token=this.epoch,abort=new AbortController();this.request=abort;this.deps.status(PREPARING);try{
  const blob=await this.deps.load(event,abort.signal);if(token!==this.epoch)return;
  const player=this.deps.audio(blob);this.playing=player;player.onEnded(()=>{if(token===this.epoch){player.dispose();this.playing=null;this.deps.status(REPLAY)}});
