...
interface Props {selected:string;minute:number;position:string;journey:Journey|null;onSelect:(id:string)=>void}
/** Isolated presentation adapter. Only whitelisted inspection messages cross this boundary. */
export function StreetScene({selected,minute,position,journey,onSelect}:Props){
 const [motion,setMotion]=useState(localStorage.getItem('black-ledger-motion')!=='off');
 const frame=useRef<HTMLIFrameElement>(null),latest=useRef({selected,minute,position,journey,onSelect,motion});latest.current={selected,minute,position,journey,onSelect,motion};
 function sync(){frame.current?.contentWindow?.postMessage({type:'blackledger:presentation',selected:latest.current.selected,minute:latest.current.minute,motion:latest.current.motion,position:latest.current.position,journey:latest.current.journey?{from:latest.current.journey.from.id,to:latest.current.journey.to.id}:null},location.origin)}
 ...
 useEffect(sync,[selected,minute,motion,position,journey]);
