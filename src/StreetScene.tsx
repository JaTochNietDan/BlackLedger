...
export function StreetScene({selected,minute,onSelect}:Props){
 ...
 useEffect(()=>{const receive=(e:MessageEvent)=>{if(e.source!==frame.current?.contentWindow||e.origin!==location.origin)return;if(e.data?.type==='blackledger:ready')sync();if(e.data?.type==='blackledger:inspect'&&['bar','club'].includes(e.data.location))latest.current.onSelect(e.data.location)};window.addEventListener('message',receive);return()=>window.removeEventListener('message',receive)},[]);
 useEffect(sync,[selected,minute,motion]);
 return <div className="street-scene"><iframe ref={frame} title="Old Harbor illustrated street" src="/street-study.html?embed=1" onLoad={sync}/><div className="street-inspect"><button aria-pressed={selected==='bar'} onClick={()=>onSelect('bar')}>Saint Agnes</button><button aria-pressed={selected==='club'} onClick={()=>onSelect('club')}>The Monarch</button><button aria-pressed={motion} onClick={()=>{localStorage.setItem('black-ledger-motion',motion?'off':'on');setMotion(!motion)}}>{motion?'Pause ambience':'Resume ambience'}</button></div></div>
}
