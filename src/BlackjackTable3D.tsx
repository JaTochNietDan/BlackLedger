...
import {poseBlackjackDealer} from './blackjackDealer';
...
export function BlackjackTable3D({mine,theirs,hidden,presentation,dealer}:{dealer?:Presence;mine:Card[];theirs:Card[];hidden:number;presentation:{plan:ReturnType<typeof planCards>;start:number;active:boolean}}){
 ...
 useEffect(()=>{
  ...
  const camera=new THREE.PerspectiveCamera(38,1,.01,20);camera.position.set(0,3.8,3.5);camera.lookAt(0,dealer?1.02:.72,dealer?-.18:0);
  ...
  const resize=()=>{const w=el.clientWidth,h=el.clientHeight;renderer.setSize(w,h);camera.aspect=w/h;camera.position.set(0,3.8,3.5).multiplyScalar(Math.max(1,1.35/camera.aspect));camera.lookAt(0,dealer?1.02:.72,dealer?-.18:0);camera.updateProjectionMatrix();dirty=true;};
