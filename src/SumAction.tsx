...
export function SumAction({a, money, disabled, commit}: {
  a: Action;
  ...
  const bad = over ? `More than the ${money(sum.most)} there is` : under ? `At least ${money(sum.least)}` : '';

  return <div className={'action sum-action' + (a.disabled ? ' refused' : '')}
    title={[a.detail, a.disabled ? a.reason : ''].filter(Boolean).join(' — ')}>
    <strong>{a.label}</strong>
    <span className="meta">{a.minutes ? `${a.minutes} min` : ''}<span>{money(sum.least)}–{money(sum.most)}</span></span>
    <span className="desc">{a.reason || a.detail}</span>
    <div className="sum-field">
      <label>
        <span>{sum.label}</span>
        <input type="number" inputMode="numeric" min={sum.least} max={sum.most} step={5}
          value={typed} disabled={a.disabled || disabled}
          aria-label={`${sum.label} — ${money(sum.least)} to ${money(sum.most)}`}
          onChange={e => setTyped(e.target.value === '' ? '' : Math.floor(Number(e.target.value)))}/>
      </label>
      <button type="button" className="plain" disabled={a.disabled || disabled}
        onClick={() => setTyped(sum.most)}>All</button>
    </div>
    <button type="button" className="action-commit" disabled={a.disabled || disabled || !!bad}
