...
export function TravelPresentation({journey, onDone}: {journey: Journey; onDone: () => void}) {
  ...
  const start = {x: from.x, y: from.y + 24},
    end = {x: to.x, y: to.y + 24};
  // A schematic route, not autonomous pathfinding or simulation.
  const corner = {x: end.x, y: start.y};
  const a = Math.abs(corner.x - start.x),
    b = Math.abs(end.y - corner.y),
    distance = (a + b) * progress;
  ...
  return (
    <div className="travel-presentation">
      <svg viewBox="0 0 1080 740" aria-hidden="true">
        <path
        ...
        <g transform={`translate(${x},${y})`}>
          <circle r="15" fill="#142522" stroke="#f2d398" strokeWidth="2" />
          <circle cy="-4" r="4" fill="#f2d398" />
          <path d="M-6 7Q0-4 6 7" stroke="#f2d398" strokeWidth="3" fill="none" />
        </g>
      </svg>
      <div className="travel-caption" role="status">
        <div>
          <b>{to.name}</b>
          <small>{minutes} game minutes · journey saved</small>
        </div>
        <button className="plain" onClick={() => done.current()}>
          Skip journey →
        </button>
      </div>
    </div>
