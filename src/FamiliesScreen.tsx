...
export function FamiliesScreen({world, onMeet, actions = [], render}: {
  ...
  const card = (f: Snapshot['factions'][number]) => {
    ...
    return <article className={'family-card' + (f.yours ? ' yours' : hostile ? ' hostile' : warm ? ' warm' : '')} key={f.id}>
      <header>
      ...
      {!!f.holdings?.length && <p className="family-ground">
      ...
      {truce[f.id] !== undefined && <p className="family-note">A ceasefire holds over business, and no further.</p>}
  ...
  return <section className="section-content">
    ...
    <p className="subtle">The city does not scale its dangers to your experience. Some doors are better left unopened.</p>

    {!!world.known_threats?.length && <section className="known-threats" style={{margin: '0 0 22px'}}>
      <strong>Reported threats</strong>
      {world.known_threats.map((t, i) => <p key={i}>{t}</p>)}
      <p>Business ceasefires do not settle personal threats. An audience may offer separate terms.</p>
    </section>}
    ...
    {!!world.conflicts?.length && <>
      ...
      <ul className="conflict-list">
        {world.conflicts.map((c, i) => <li key={i} className={c.state}>
          <b>{c.between.join(' and ')}</b>
          <span>{c.state === 'war' ? 'at war' : 'at odds'} since day {Math.floor(c.since / 1440) + 1}</span>
        </li>)}
      </ul>
    </>}
  </section>;
