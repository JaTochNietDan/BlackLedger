...
type World struct {
	...
	// Which of the two things in the room they sat down to: the floor, or the
	// room behind it. Absent in saves written when a seat was only an address.
	SeatedTo string `json:"seated_to,omitempty"`
	...
	PolicePresence []PolicePresence `json:"police_presence"`
...
func (w *World) Actions(id string) []Action {
	...
	if Playable(id) && id == p.Location {
		if w.Seated == id {
			...
			// Two of these rooms have both a floor and a room behind it, so
			// both are offered. One button that guessed put a player who
			// asked for the machines into a hand of cards.
			if HasTables(id) || HasMachines(id) {
				...
				add("sit:"+Floor, seat, 0, 0, w.SitReadiness(id), what)
			}
			if HasBackRoom(id) {
				add("sit:"+Backroom, "Go through to the back room", 0, 0, w.SitReadiness(id),
					"A hand of cards against whoever is in there, played out in front of you until you get up.")
			}
		}
	}
	...
	if HasTables(id) && !w.Own(id) {
		...
		// The dice. Neither of the other two is this shape: one decision, and
		// then a run of rolls nobody at the table can affect and everybody has
		// to sit through.
		if w.Dice != nil && !w.Dice.Done && w.Dice.Place == id {
			add("roll", fmt.Sprintf("Throw again for the %d", w.Dice.Point), 5, 0, w.RollReadiness(),
				fmt.Sprintf("$%d is already on %s. %s", w.Dice.Down, lowerFirst(diceBetLabel(w.Dice.Bet)), w.Dice.Outcome))
		} else {
			asks("dice", "Play the dice", 30, 0, w.DiceReadiness(id, "pass", Stake{Amount: bet}),
				fmt.Sprintf("Anything from $%d to $%d on the line. The pass line and the don't both pay even money and the house keeps about 1.4 in every hundred, which is the best price in the building; the field looks like most of the numbers and keeps nearly twice that.",
					LeastStake, w.TableLimit(id)))
		}
	}
