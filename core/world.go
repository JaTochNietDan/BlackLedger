...
func (w *World) Attack(plot Plot) {
	...
	if plot.Known || w.Guard() > 0 || p.Contacts >= 2 {
		...
		w.Event = &Scene{ID: "attack-" + plot.ID, Title: "Headlights outside", Body: body + " You have moments to act.", Speaker: "mara", Kind: "attack", Source: "authored", Minute: w.Minute, Choices: []Choice{{ID: "escape", Label: "Leave through the rear", Detail: "A chance to escape. Security and contacts improve your odds."}, {ID: "defend", Label: "Hold the entrance", Detail: "Rely on your security. A weak defense may be fatal."}, {ID: "bargain", Label: "Offer $180 to stand down", Cost: 180, Detail: "Money may settle this incident, but your standing suffers."}}}
	} else if w.Random() < .78 {
...
func (w *World) ValidateProposal(p Proposal) (*Scene, error) {
	...
	return &Scene{ID: ID(), Title: p.Title, Body: p.Body, Speaker: p.Speaker, Kind: "proposal", Source: "local-ai", Minute: w.Minute, Effect: fx, Outcome: p.Outcome, Choices: []Choice{{ID: "accept", Label: "Accept the arrangement", Detail: fmt.Sprintf("$%d · %d minutes · +%d respect · +%d heat", fx.Reward, fx.Minutes, fx.Respect, fx.Heat)}, {ID: "decline", Label: "Decline the arrangement", Detail: "No cost or time."}}}, nil
}
