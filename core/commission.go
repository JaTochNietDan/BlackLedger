...
func (w *World) AvailableCommission(location string) (Commission, bool) {
	...
	if rival := w.fighting(f.ID); rival != nil {
		...
		if rival.Cash > f.Cash {
			...
			c.Brief = fmt.Sprintf("Take nine hundred dollars out of %s. %s does not care how it leaves their hands, only that it does.", rival.Name, f.Name)
		...
		if holdings := w.FamilyHoldings(rival.ID); len(holdings) > 0 {
			...
			if prop.Condition > 45 {
				...
				c.Brief = fmt.Sprintf("Put %s out of the state it is in. %s wants it at forty-five percent or worse, and does not want to be seen doing it.", place.Name, f.Name)
	...
	if richest := w.richest(f.ID); richest != nil && f.Cash < richest.Cash {
		...
		c.Brief = fmt.Sprintf("Bring twenty-five crates of moonshine to %s and hand them over. %s is short and would rather owe you than owe anybody else.", place.Name, f.Name)
