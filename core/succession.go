...
func (w *World) CoupAftermath(f *Faction, winner, loser *NPC) {
	...
	w.Report("politics", "UPHEAVAL IN "+upper(f.Name),
		fmt.Sprintf("%s %s understood to be under new leadership following the death of %s. Associates describe the organization as divided.", Leads(f.Name), Agree(f.Name, "is", "are"), loser.Name))
}
