...
New live reports capture every provider attempt, including rejected final-answer content, HTTP status, timing, token counts and a SHA-256 fingerprint of the request. The test-only proxy passes response bytes/status and cancellation through to normal generation. Reasoning text is not retained. Reports also identify the brief mode. A regression test verifies failure-response preservation and omission of reasoning. Earlier reports do not gain these records retroactively.
