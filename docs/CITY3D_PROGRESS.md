...
### Recorded weapon models, two-hand handling and replay fixes — September 13

- Authored pump-shotgun and Thompson GLBs in Blender: packed walnut grain, blued receivers/barrels, sights, trigger guards, pump grooves or magazine/cooling fins, plus muzzle/support anchors. Added elbow joints to the person/woman rigs and regenerated measured stride bounds. The two-segment arm solver reaches both long-gun grips through aiming, recoil and pumping without stretching limbs.
- Gun scenes now use recorded attacker appearance and equipment. Shotgun cadence is two shots with a moving forearm/support hand between them; Thompson cadence is two three-shot bursts. Both muzzle pulses and existing procedural shot audio follow that cadence. Dedicated per-weapon sound timbre, shell ejection and reloads remain unfinished.
- Browser QA caught a duplicate public player alongside its scene actor. The staged actor now replaces that street representation, and its deliberate hiding is excluded from the waiting-traveller count. QA also found rapid Go on/replay could miss restarting the same cue; a replay serial now identifies presentation starts without altering saved IDs.
- Production build and 178 frontend tests pass, with expanded actual-GLB checks for both characters, both long guns, grip reach throughout playback, whole-cast reservation clearance, pump movement, cadence/audio beat counts and the existing stride/casualty suite. Export validation initially caught missing motion-bound metadata in the partial export; regenerated it before acceptance.
- Browser isolated 8871 shotgun fixture selected player/tier 2 and showed its first shot with the long gun. Isolated 8872 Thompson fixture selected tier 3, emitted six shot sounds and displayed one attacker. Final browser check showed no false waiting actor, restored the public player after completion, and restarted both effects on rapid Go on/replay. Evidence: `thompson-scene.png`, `weapon-scenes.json`. Local expanded sample 145 FPS/296 draws. Main save untouched; sleep prevention active.
- Read amended objective `/Users/jatochnietdan/.codex/attachments/02dfee88-dcac-4620-8521-e333b4f5c6dc/goal-objective.md`. It adds priority to an unsuspecting target and a walking assassin approaching from behind for one shot, plus variable scenarios. The current weapon poses are a foundation; that approach/attack choreography, regular NPC weapon progression, other combat cue producers, full interior/art quality and broader production acceptance remain unfinished.

### Authoritative strike attacker and equipment — September 13

- Traced player/crew weapon ownership through delegation, strike resolution and witness records. Successful strikes previously always emitted anonymous gunfire and called the random location-based manner generator, which could describe an armed shooting as a knife attack. They now snapshot an explicit attacker and actual weapon tier, use weapon-consistent death descriptions, and emit `attack` for unarmed successes. The same world-random manner selection draw is retained.
- Added optional public/TypeScript `attacker: {id,name,weapon}`. Tests cover both player and crew across all four weapon tiers, crew equipment independent of player equipment, armed/unarmed cue kinds, consistent death captions and saved event equipment unchanged by later gear changes.
- Production frontend build, targeted strike tests, full core suite (72.9s), save suite and HTTP adapter suite pass. These runs include the pre-existing unrelated armed-resistance worktree changes; those files remain excluded from this commit.
- Added isolated `strike-revolver`, `strike-shotgun`, `strike-thompson` fixtures that execute an actual successful strike. Saved all three fresh fixtures under `.runtime/strike-*-20260913.sqlite3`; new Go binary `.runtime/weapon-cue-server` serves shotgun fixture on 8871. HTTP evidence `weapon-cues.json` shows player/tier 2 and a pump-shotgun death description at revision 0/minute 480. Main campaign untouched.
- Renderer gun/model selection remains generic and must now consume this evidence; long-gun Blender assets, weapon-specific handling/audio, regular NPC ownership/progression and other combat producers still require work. This checkpoint corrects event truth, not completion of weapon visual fidelity.

### Pressure-timed glazing break and sound — September 13

- Glazing now changes at 90ms in the internal blast timeline, using known pre-action condition. Already broken panes stay broken; debug explosions temporarily break intact panes and restore the saved variant on Stop/completion. Added a six-layer procedural crack/scatter sound owned by the cancellable scene audio lifecycle; this is an initial sound design, not final recorded-glass fidelity.
- Real-action browser QA found that entering the city from an interior remounts the renderer and loses its former building state. The action handler now retains a world/revision-scoped condition map for presentation; City3D consumes it across that handoff and same-session replay. Public backend semantics are unchanged.
- Production build and 175 frontend tests pass. Timing tests cover the 90ms boundary, prior damage and preview restoration; the audio lifecycle test now includes all six glass voices and cancellation. Exact onset timing is unit-verified: browser tool latency exceeded the pre-break window.
- Browser 8868 repaired laundry: intact before debug playback, broken with one glass onset during it, intact with no active effects after Stop. Fresh actual-action fixtures recorded the original missing-glass-sound failure on 8869, then the corrected UI planting action on 8870 (`.runtime/glass-break-remount-20260913.sqlite3`): interior-to-city transition, one explosion and one glass onset, persistent broken glazing afterward. Evidence: `glass-break-timing.json`. Main campaign untouched, sleep prevention active.
- Glass panes currently switch variants; individual flying glass shards and polished breakage choreography still need production, alongside broader art, interiors, weapon fidelity, vehicle/action choreography and audio goals.

### Authored shattered glazing and repair restoration — September 13

- Blender building production now exports separate intact and broken glazing groups, batched by material. Broken panes retain four jagged glass remnants around a dark recessed backing; their centre glazing is missing. Updated all ten standard building GLBs and manifest byte counts. Existing parcel bounds, fire anchors and debris corridor checks remain passing.
- Front upper windows (front ground windows on single-storey models) use the broken variant below 60% condition and restore intact panes when repairs raise condition to 60% or more. Cleanup and extinguishing leave glazing damage alone. This is a condition-based visual state, not a claim that all damage originated in an explosion.
- Production build and 174 frontend tests pass. Tests cover condition boundaries/repair restoration and raycast the exported intact pane versus the broken variant's missing centre across all ten models.
- Browser isolated 8867 revision 4/minute 840: The Monarch at 38% retains broken panes after fire/rubble cleanup; expanded screenshot `broken-glazing.png`, local 145 FPS/7.1ms p95. Fresh owned-property fixture on 8868 (`.runtime/glazing-repair-20260913.sqlite3`) rendered broken Bluebird Laundry panes at 45%; an actual repair command restored 85% at revision 1/minute 540 and browser telemetry confirmed intact glazing. Evidence: `broken-glazing.json`. Main campaign untouched.
- Variants currently repeat the same jagged pattern and represent a dark recess rather than a fully opened interior. Breakage animation, glass ejection/audio, other model families, richer architectural damage and broader production fidelity remain unfinished.
...
- Matching public fire target/minute distinguishes an actual internal detonation from an early charge accident. Debug explosion explicitly selects the internal sequence. Older snapshots without confirmation conservatively retain their prior exterior effect. Models without authored window anchors still need emitter production.
- Production build and 170 frontend tests pass (plus a TypeScript optional-minute correction caught by the production build). New tests cover the inside/outside transition, finite trajectories, three-second completion, and accident/other-address/other-minute gating.
- Browser review on isolated 8867: debug burst emits from The Monarch's four upper windows; captured internal source is x112/y5.13/z74.73 with glazing at z74.28. A fresh final-build page replayed the committed killing/explosion pair and selected the same internal source and four windows, with one explosion audio onset. Existing fire and brigade continue afterward. Evidence: `window-blast.png` and `window-blast.json`. This is a choreography improvement; final effects fidelity, arrival/departure, window breakage and broader game production acceptance remain unfinished. Main campaign untouched.
...
- Suppression follows saved arrival/extinguishing times, freezes particle motion when requested, and releases hose/stream geometry and restores the standing pose on completion. The first implementation has fixed working poses and dotted water streams; deployment, impact spray, hose reeling and more adaptive routing remain unfinished.
- Production build and 167 frontend tests pass. New tests cover arc endpoints/facade bounds, absent/hidden brigade gating, motion freeze, disposal and arm reset at extinguishing. Browser isolated 8866 at revision 1/minute 720 showed two nozzle-to-window streams, connected hoses and no waiting actors. Corrected subpixel water sizing for the orthographic camera. A real hour-pass reached revision 2/minute 780 with no suppression lines and three brigade objects retained. Evidence: `fire-hoses.png` and `fire-hoses.json`; local close-view HUD showed 145 FPS/257 draws. Main campaign untouched.
- This advances firefighting presentation but does not complete full fire/action fidelity, arrivals/departures or the broader game goal.

### Firefighter cast and stationary parking fix — September 13

- Added a Blender-authored firefighter variant with broad helmet/brim/shield, dark baked woven coat, storm flap and brass clasps, retaining the articulated limb hierarchy. Two crew members face the fire from separate forecourt positions and remain through brigade cleanup. This is standing attendance; no hose handling or search animation is claimed yet.
- Integrated browser QA exposed an initial-load parking defect: response staging considered only visible actors and could occupy the not-yet-visible player's parked-car bay. Scene and aftermath staging now share occupancy from the known destinations of stationary actors, even before first visibility. The renderer test includes an occupied Hudson bay alongside police, a body, the engine and both firefighters.
- Production build and 165 frontend tests pass. Actual exported firefighter geometry fits standing clearance and retains limbs; response lifecycle and combined spacing tests pass. Fresh isolated 8866 (`.runtime/fire-crew-20260913-1815.sqlite3`) after an actual planting command is revision 1/minute 720. Browser replay/reload showed crew at x106 and x118/z70.35, engine x122/z86, player car and police all visible with no waiting actors. Evidence: `fire-crew.png` and `fire-crew.json`. Rain in this fixture is the public weather projection. Main campaign untouched.
- Hose/nozzle authoring, connected water streams, working crew poses and arrival/departure choreography remain required, as do the broader production goals.

### Authored fire engine and response attendance — September 13

- Added a locally authored period-style fire engine with rounded bonnet, split windscreen, metal grille, pump fittings, wheels/lugs, folded hose bed and side ladders. Source/export are in the existing Blender pipeline. The first model uses material finishes; richer texture/weathering and articulated travel remain unfinished.
- Saved `building_fires` now stages one engine from brigade arrival until cleanup, retaining it after flames stop. Shared aftermath traffic reservations use a dedicated 5.8m by 2.35m footprint and 10m side-bay offset. Tests measure the exported GLB against that footprint and verify separation from a co-located death scene and its police response, response timing and exact cleanup.
- Production build and 164 frontend tests pass. Browser isolated 8865 at revision 2/minute 780 showed engine x122/z74 visible beside The Monarch, with no remaining fire; the close screenshot shows separation from the parked Hudson, hose bed and ladders, and 145 FPS/326 draws in the local HUD. A real hour-pass reached cleanup at minute 840 and opened a rival encounter; paying its $180 demand in this test save restored the city at revision 4/minute 840 with no aftermath objects. Evidence: `fire-engine.png` and `fire-engine.json`. Main campaign untouched.
- This is the vehicle/attendance stage, not completed firefighting. Crew models, arrival/departure, hose connections, water streams and extinguishing choreography remain required alongside the broader production-quality goal.

### Persistent window fire rendered — September 13

- Added authored front-window emitter nodes to ten Blender building models (tenement, tavern, casino, warehouse, civic, shop and four named venues). Added one instanced particle draw per active building, with four windows and 48 looping flame/smoke billboards. The renderer freezes motion when disabled and releases instances exactly at the saved extinguishing minute. Upper windows are preferred where available: browser inspection found entrance canopies hid the ground-floor flames.
- An actual isolated planting command exposed a lifecycle timing defect: its 120 minutes consumed the initial +45/+90 response window before returning. Revised extinguishing/departure to +180/+240, retaining +10 dispatch. A command-level regression test now verifies a successful detonation is still burning after the planting duration. This keeps existing command timing/death semantics intact.
- Production build, 162 frontend tests and focused Go fire tests pass. Tests check actual exported anchors have window geometry immediately behind them, persistent object reuse during paused game time, frozen motion, disposal at deadline and full planting-command behavior. Browser fixture on 8865 (`.runtime/fire-visible-20260913-1800.sqlite3`) reached revision 1/minute 720 after planting: The Monarch at 38% condition had four upper-window vents and 48 particles after replay/reload. A real hour-pass reached revision 2/minute 780: no fire particles and condition still 38%. Evidence: `window-fire.png` and `window-fire.json`. Earlier 8864 fixture exposed the short-deadline failure and remains isolated. Main campaign untouched.
- These are initial procedural fire billboards. Fire-brigade models/arrival/hose action, internally originated blast choreography, richer smoke/flame art, industrial/specialist emitters and full debug-fire lifecycle remain unfinished. Broader production goal remains active.

### Saved building-fire lifecycle — September 13

- Traced successful building detonations separately from early charge accidents. Added `building_fires` only to the actual building-detonation path, independently of transient cues/results. Records persist game-minute arrival/extinguishing/cleanup deadlines (+10/+45/+90). Repeated real detonations renew the fire while retaining the original brigade response; same-minute ignition cannot extend deadlines.
- Focused Go tests pass against actual detonation behavior, saved JSON restoration, detached public projection, result replacement, renewal and exact cleanup. Extinguishing preserves authoritative property damage. Save/HTTP adapter suites and TypeScript check pass (`.runtime/building-fire-core.log`, `building-fire-boundary.log`, `building-fire-types.log`). No campaign-save access or browser-state changes.
- This is the saved simulation foundation. The frontend has the typed projection but still needs persistent window fire/smoke, internally originated blast assets, fire-engine/crew models and extinguishing choreography. No visual completion is claimed for this checkpoint; the full goal remains active.
...
- Seven-second raid cast playback now includes a lead officer approaching the aligned door, a first kick pose, inward leaf opening and entry only after the leaf clears. Supporting officers hold outside. The full movement reserves a corridor; saved raid presence holds the door open after playback. Other building types retain the prior approach pending authored openings. This is an initial sequence, not final force/impact animation or a complete squad search.
- Browser QA found and fixed two defects: an oversized reservation blocked the lead against the stationary player's entrance position, and the visibility aid dissolved the whole target once the officer entered. The corrected reservation encloses the motion while clearing the public entrance; authored raid targets retain their facade. Tests cover the regression, door-before-entry timing and actual GLB passage/floor geometry with the leaf open and closed.
- Production build and 158 frontend tests pass. Final isolated 8860 preview kept all seven effects staged, door angle reached -PI/2 before entry, and lead z advanced from 38.35 through the threshold at 41.92 to 43.17. Final screenshot shows the facade intact and officer entering, with local 145 FPS/274 draws shown in the HUD. Stop cleared effects and reset the door to zero. No gameplay command was issued; revision 3/minute 660 unchanged. Evidence: `raid-entry.png` and `raid-entry.json`.
- Remaining: stronger whole-body breach motion, door contact sound/debris, squad follow-through/search, seamless transition to lingering attendance, additional door models and the broader fire/weapon/interior/action quality requirements. Goal remains active.

### Saved raid cordons — September 13

- Added independent saved/public `police_presence` from actual raid witnesses, with one cordon per address and a 120-game-minute cleanup deadline. Result replacement, page reload and replay do not erase or extend it. Later real raids can extend attendance. Invalid addresses and arrest cues cannot invent raid cordons; old saves retain an empty projection.
- The city keeps three cars and four uniformed officers after playback, sharing the existing spacing/reservation and disposal paths. Playback temporarily removes the persistent cast and restores it afterward. This first attendance pass uses standing forecourt guards; seamless movement from approach endpoints, forced entry, search and departure still need animation. Facades are currently solid meshes, so real door openings must be authored before entry can avoid wall intersections.
- Production build, all 155 frontend tests, targeted Go raid-presence tests, and the save/HTTP adapter suites pass. Tests cover saved JSON restoration, detached public reads, duplicate/repeated raids, exact cleanup, seven separated reservations, replay suppression and removal. Browser fixture on isolated 8863 (`.runtime/raid-presence-20260913-1740.sqlite3`) showed seven visible persistent objects at minute 480; replay showed seven effects and zero persistent duplicates, then restored all seven. A real hour-pass command and fresh page showed seven at revision 1/minute 540. A second hour-pass reached revision 2/minute 600 with no persistent objects or effects. Evidence: `raid-presence.png` and `raid-presence.json`. Main campaign untouched; Mac sleep prevention remains running.
- Broader production acceptance remains unfinished, including authored door entry, persistent window fire/fire brigade, detailed interiors, weapon fidelity and layered action/audio choreography.
