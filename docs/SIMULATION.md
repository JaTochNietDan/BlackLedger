...
The optional `diplomat` policy follows investor progression but negotiates expired/missing business ceasefires for owned-business districts when it has at least $200. It reads only the public expiry map and displayed action availability; active agreements are not repurchased. Audience decisions select the $100 ceasefire or leave if unaffordable. This is a reproducible policy, not an optimal strategy.

`go run ./cmd/simulate -runs 50 -steps 220 -strategies investor,diplomat -director authored` exercised 100 campaigns in 6.40 seconds with zero errors. Both policies reached casinos in all 50 runs: median command 61 for diplomat versus 56 for investor. Neither died. Final money totals are not directly comparable income rates because the policies advanced different amounts of game time. See `diplomacy-simulation-summary.json`.
