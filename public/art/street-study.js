...
const assets={};let t=0,last=0,frame=0,selected='',arrival=null,playerLocation='',journey=null,journeyKey='';
...
try{await Promise.all([...buildings.map(b=>[b.id,b.file]),['car','sedan-noir-v1.png'],['asphalt','asphalt-noir-v1.png']].map(async([id,file])=>{const img=new Image();img.src='/art/'+file;await img.decode();assets[id]=img}));asphalt=ctx.createPattern(assets.asphalt,'repeat');asphalt.setTransform(new DOMMatrix([.28,.14,-.28,.14,0,0]));new ResizeObserver(resize).observe(canvas);resize()}catch(e){status.className='error';status.textContent='An art asset could not load. Reload this study to retry.'}

if(embedded){window.addEventListener('message',e=>{if(e.source!==parent||e.origin!==location.origin||e.data?.type!=='blackledger:presentation')return;const data=e.data;playerLocation=typeof data.position==='string'?data.position:'';const key=data.journey?data.journey.from+'>'+data.journey.to:'';if(key!==journeyKey){journeyKey=key;const from=buildings.find(b=>b.id===data.journey?.from),to=buildings.find(b=>b.id===data.journey?.to);journey=key&&to&&!reduced.matches?{route:streetRoute(from,to),start:performance.now()}:null}selected=buildings.some(b=>b.id===data.selected)?data.selected:'';if(Number.isFinite(data.minute)){const hour=(data.minute%1440)/60;light.value=String(Math.round((hour<6||hour>=20?1:hour<8?(8-hour)/2:hour>17?(hour-17)/3:0)*100))}if(typeof data.motion==='boolean')motion.checked=data.motion&&!reduced.matches;wake()});parent.postMessage({type:'blackledger:ready'},location.origin)}
