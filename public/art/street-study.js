...
function artFailure(){if(embedded)parent.postMessage({type:'blackledger:art-error'},location.origin)}
...
function sceneActors(objects,n){
 if(!sequence)return;
 const b=buildings.find(b=>b.id===sequence.target);if(!b)return;
 const elapsed=(performance.now()-sequence.start)/1000;
 // Keep cars on one of the authored roads, never on a straight line through buildings.
 const x=b.door[0],roadA=.5*x+320,roadB=-.5*x+790;
 const slope=Math.abs(roadA-b.door[1])<Math.abs(roadB-b.door[1])?.5:-.5;
 const stop=[x,slope>0?roadA:roadB];
 const entry=[stop[0]-680,stop[1]-680*slope];
 const leave=[stop[0]+680,stop[1]+680*slope];
 const point=elapsed<1.3?routePoint([entry,stop],elapsed/1.3):elapsed<3.2?stop:routePoint([stop,leave],(elapsed-3.2)/1.3);
 objects.push({depth:point[1]+16,draw:()=>{car(...point,n,slope<0);glow(point[0]+30,point[1]+10,38,.22)}});
 if(elapsed>1.3&&elapsed<3.2){const p=routePoint([stop,b.door],Math.min(1,(elapsed-1.3)/.55));objects.push({depth:p[1]+2,draw:()=>{person(p[0],p[1],0,n);person(p[0]+12,p[1]+6,3,n)}})}
}
...
function tick(now){frame=0;if(document.hidden)return;const dt=last?Math.min(.05,(now-last)/1000):0;last=now;if(motion.checked)t+=dt;render();if(motion.checked||journey||sequence)frame=requestAnimationFrame(tick)}
