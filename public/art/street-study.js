...
function ground(n){ctx.fillStyle=n>.5?'#253133':'#55554a';ctx.fillRect(0,0,1280,820);
...
document.querySelector('#cafe').onclick=()=>inspect('bar');document.querySelector('#casino').onclick=()=>inspect('club');document.querySelector('#laundry').onclick=()=>inspect('laundry');light.oninput=wake;motion.onchange=wake;document.querySelector('#arrival').onclick=()=>{if(reduced.matches){status.textContent='Arrival preview skipped because reduced motion is enabled.';return}motion.checked=true;arrival={start:t,reported:false};status.textContent='A car is approaching The Monarch. Preview only; no game time passes.';wake()};document.addEventListener('visibilitychange',wake);reduced.addEventListener('change',()=>{motion.checked=!reduced.matches;wake()});
